  ...
  return <Card className="bg-gray-800 border-gray-700">
      ...
      <CardContent className="space-y-6 pt-6">
        ...
        <div className="flex items-baseline gap-2">
          <span className="text-white font-normal text-3xl">$9.99</span>
          <span className="text-gray-400 text-xl">/month</span>
        </div>

        {profile?.subscription_status === 'trialing' && <Alert className="bg-red-900/40 border border-red-500/30">
            <Calendar className="h-5 w-5" />
            <AlertDescription className="text-white text-base">Important: After your 3-day trial, you will be charged $9.99/month.</AlertDescription>
          </Alert>}
        ...
        {profile?.is_subscribed ? <Button onClick={handleManageSubscription} disabled={isLoading || isLoadingState} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-6 text-lg">
            {isLoading || isLoadingState ? "Loading..." : "Manage Subscription"}
          </Button> : <Button onClick={() => window.location.href = '/premium'} disabled={isLoading} className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-medium py-6 text-lg">
            Subscribe Now
          </Button>}
      </CardContent>
    </Card>;
