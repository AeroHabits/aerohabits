...
export function AddHabitForm({
  ...
  const fetchCategories = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from('habit_categories').select('*').eq('user_id', user.id).order('name');
    if (error) {
  ...
  return <motion.form onSubmit={handleSubmit} initial={{
    ...
    duration: 0.4
  }} className="space-y-6 bg-gradient-to-br from-[#1F2A47] to-[#1A1F2C] backdrop-blur-lg p-8 shadow-2xl border border-[#403E43]/50 hover:border-[#9b87f5]/50 transition-all duration-300 rounded-xl">
      <FormHeading />
