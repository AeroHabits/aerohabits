...
import { SYNC_QUEUE_KEY, BATCH_SIZE } from "./syncConstants";
import { HabitSyncQueueItem } from "./syncTypes";
import { useRetryLogic } from "./retryUtils";
import { useOnlineStatus } from "../useOnlineStatus";

export function useSyncProcessor() {
  const isOnline = useOnlineStatus();
  const [isSyncing, setIsSyncing] = useState(false);
  const { scheduleRetry } = useRetryLogic();
  ...
  }, [isOnline, isSyncing, scheduleRetry]);
